...
const cloudfront = new (require("aws-sdk/clients/cloudfront"))({
  accessKeyId: credentials.accessKeyId,
  secretAccessKey: credentials.secretAccessKey,
  region: credentials.region
});
...
gulp.task("handlebars", function(done) {
    ...
    return new Promise((resolve, reject) => {
      fs.readFile("client/src/index.hbs", "utf-8", function(error, source) {
        ...
        var html = template({
          ...
          metaImg:
            "https://s3.amazonaws.com/cdn.sonyafalcon.com/2018/01/22/reddddd.jpg"
        });
        resolve(html);
      });
    });
  ...
  function write(html) {
    return new Promise((resolve, reject) => {
      fs.writeFile(`client/dist/index-${timestamp}.html`, html, function(
        error,
        source
      ) {
        if (error) reject(error);
...
gulp.task("cloudfront", function(done) {
  getCloudFrontId()
    ...
    .then(() => {
      console.log(
        `CloudFront Distribution ${credentials.cloudFrontDistribution} updated.`
      );
      done();
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.readFile(
        ...
        (error, source) => {
          ...
          if (!post.page) {
            post.formattedTimeAttr = moment(post.timestamp).format(
              "YYYY-MM-DD"
            );
                ...
                s3.putObject(
                  ...
                  err => {
                    if (err) reject(err);
                      ...
                      resolve({
                        msg: `Successfully rendered post '${post.id}'`
                      });
                  }
                );
              }
    ...
    return cloudfront
      ...
      .then(distributions =>
        _.find(distributions.DistributionList.Items, {
          DomainName: credentials.cloudFrontDistribution
        })
      );
  }
  ...
  function updateDistribution(distribution) {
    const getDistributionConfig = () => {
      return cloudfront
        ...
        .then(({ ETag, DistributionConfig }) => {
          let oldTimestamp = DistributionConfig.DefaultRootObject.replace(
            "index-",
            ""
          ).replace(".html", "");
    ...
    return getDistributionConfig().then(({ ETag, DistributionConfig }) => {
