...
export const ImagePlaceholder = ({ openMediaModal }) => (
  <div style={{ height: "100%", width: "100%" }}>
    <AddMediaButton style={{ height: "auto", width: "100%" }} />
    <em style={{ width: "100%", idsplay: "inline-block", fontSize: 10 }}>
...
export class MediaInput extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      cursor: "grab",
      contents: (
        <ImagePlaceholder openMediaModal={this.openMediaModal.bind(this)} />
      ),
      background: "white",
      files: [],
      isModalOpen: "modal"
  ...
  componentDidMount() {
    this.setState({
      ...
      contents: <ImagePreview src={this.state.value} />
    });
  }

  componentWillUnmount() {
    if (this.state.files && this.state.files.forEach)
    ...
    if (files.length) {
      ...
      files.forEach(file => {
        ...
        axios({
          ...
          url: `${
            window.config.post
          }/getUploadCredentials?filename=${filename}&type=${file.type}`,
          ...
          .then(() => {
            ...
            this.props.notify(
              `File uploaded. Access it at ${uploadedFileUrl}`,
              "success"
            );
  ...
  closeMediaModal(fileUrls) {
    ...
    if (fileUrls) {
      ...
      this.setState({
        value: fileUrls,
        contents: <ImagePreview src={fileUrls} />
      });
    } else
      this.setState({
        contents: (
      ...
      >
        <div
          style={{
            position: "absolute",
            width: "100%",
            display: this.state.dragOverCursor,
            background: "#00d1b2",
            top: -21,
            height: 14,
            borderRadius: 5
        ...
        />
        <p
          style={{ color: "red", cursor: "pointer", zIndex: 5 }}
        ...
        <MediaLibraryModal
          modalClass={this.state.isModalOpen}
          closeModal={this.closeMediaModal.bind(this)}
        />
      </div>
    ) : null;
  }
  ...
  render() {
    return this.state ? (
      <div
        ...
        style={{
          flex: 1,
          maxWidth: this.props.first ? "70%" : "100%",
          borderRadius: 5,
          position: "relative",
          margin: "14px 0",
          cursor: this.state.cursor
