    ...
    <PostContainer style={{ width: screenIs("mobile") ? "100%" : "85%" }}>
      ...
      {post.page ? null : (
        <time
          ...
          dateTime={moment(post.timestamp).format("YYYY-MM-DD")}
        >
        ...
        {post.layout.map((section, index) => {
          return section.type === "markdown" ? (
            <section
              ...
              key={index}
              style={{ margin: "14px 0" }}
              dangerouslySetInnerHTML={{ __html: markdownToHtml(section.src) }}
            />
          ) : (
            <ImageSection className="post-images" key={index}>
              {section.src.map(file => {
                ...
                if (/image/.test(mimeType))
                  ...
                  return (
                    <video
                      ...
                      style={{
                        flex: 1,
                        maxWidth: `${100 / section.src.length - 2}%`
                      }}
                    >
