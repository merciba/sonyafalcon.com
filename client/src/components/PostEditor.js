...
class PostEditor extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      addNewContent: (
        <AddContent
          onTextClick={() => this.selectNewContent("markdown")}
          onImageClick={() => this.selectNewContent("image")}
        />
      ),
      id: "",
      title: "",
    ...
    this.props
      ...
      .then(() => {
        if (/editor\/\S+/.test(window.location.pathname)) {
          ...
          return this.getPost(id).then(res => this.init(res.data.body[0]));
        } else {
          window.localStorage.setItem("currentDraft", null);
          return this.init();
        }
  ...
  onUploadImage({ index, src }) {
    ...
    if (layout[index] && layout[index].src.length)
      layout[index] = { type: "image", src: layout[index].src.concat(src) };
  ...
  addDetails() {
    ...
    details.push({
      title: detailsEditTitle,
      description: detailsEditDescription
    });
    this.setState({
      details,
      detailsEditTitle: "",
  ...
  save() {
    ...
    if (id.length && title.length && html.length && category.length) {
      let post = {
        ...
        subcategory,
        details,
        scheduled: scheduled.isBefore("second")
          ? null
      ...
      // console.log(post)
      return axios({
        method: /editor\/\S+/.test(window.location.pathname) ? "put" : "post",
        url: window.config.post,
        headers: {
  ...
  publish() {
    ...
    return axios({
      method: "post",
      url: `${window.config.post}/publish`,
      headers: {
        Authorization: window.localStorage.getItem("id_token"),
        "Content-Type": "application/json"
      },
      data: { id, timestamp: timestamp.utc().format() }
    })
      .then(res => {
    ...
    return this.state && this.state.loading ? (
      ...
      >
        ...
        <div style={{ marginTop: "22vh" }}>
          ...
          >
            <MediaInput
              onClickRemove={() => this.setState({ titleImg: "" })}
              onUpload={this.onUploadTtileImage.bind(this)}
              default={this.state.titleImg}
            />
            <FlexContainer
              style={{
                background: "#eee",
            ...
            >
              <div
                className="control"
                style={{ margin: "0 0 14px 0", width: "25%" }}
              >
                <input
                  style={{
                    color: "black",
              ...
              >
                <div style={{ margin: "10px 5px 0 0", flex: 1, width: "20%" }}>
                  From:{" "}
                  <Dropdown
                    content={[...Array(50).keys()].map(n => n + 2000)}
            ...
            <FlexContainer style={{ flexDirection: "column" }}>
              <PostContainer style={{ width: "100%" }}>
                <PostDetailsContainer>
                  {this.state.details.map(({ title, description }, index) => (
                    <PostDetailsEdit key={title}>
                      <p
                        style={{
                          color: "red",
                          position: "absolute",
                        ...
                        onClick={() => this.removeDetails(title)}
                      >
                        x
                      </p>
                      ...
                      {index === this.state.details.length - 1 ? null : (
                        <PostDetailsLine />
                      )}
                    </PostDetailsEdit>
                  ))}
                  {this.state.details.length < 3 ? (
                    <PostDetailsEdit>
                      <PostDetailsEditTitle
                        placeholder="Title"
                        value={this.state.detailsEditTitle}
                        onChange={({ currentTarget }) =>
                          this.setState({
                            detailsEditTitle: currentTarget.value
                          })
                        }
                      />
                      <PostDetailsEditDescription
                        placeholder="Description"
                        value={this.state.detailsEditDescription}
                        onChange={({ currentTarget }) =>
                          this.setState({
                            detailsEditDescription: currentTarget.value
                          })
                        }
                      />
                      <PostDetailsEditSubmit
                  ...
                  if (section.type === "image") {
                    return (
                      <MediaInput
                        ...
                        onDragEnter={({ target }) =>
                          this.onSpaceDragEnter({ target, index })
                        }
                        onDragLeave={({ target }) =>
                          this.onSpaceDragLeave({ target, index })
                        }
                        ...
                        onUpload={files =>
                          this.onUploadImage({ index, src: files })
                        }
                        default={section.src}
                      />
                    );
                    ...
                    return (
                      <MarkdownInput
                        key={index}
                        id={`section-${index}`}
                        onDrop={({ target }) => this.onSpaceDrop(index)}
                        onDragEnter={({ target }) =>
                          this.onSpaceDragEnter({ target, index })
                        }
                        onDragLeave={({ target }) =>
                        ...
                        onClickRemove={() => this.removeSection(index)}
                        onChange={text =>
                          this.onTextChange({ index, src: text })
                        }
                        default={section.src}
                      />
                    );
                ...
                >
                  ...
                  {this.state.published ? (
                    ...
                    <PublishButton
                      ...
                      className="button is-large is-primary"
                      onClick={this.saveAndPublish.bind(this)}
                    >
                      Save & Publish
                    </PublishButton>
                  )}
                  {/editor\/\S+/.test(window.location.pathname) ? (
                    <a
