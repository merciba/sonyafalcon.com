...
class Home extends Component {
  ...
  componentDidMount() {
    this.getPosts()
      .then(res => {
        let posts = _.sortBy(res.data.body, o =>
          moment(o.date).format("YYYYMMDD")
        );
        this.setState({ posts });
  ...
  render() {
    return (
      <FullScreenContainer>
        <FullScreenImage
          src={screenIs("mobile") ? LOGO.url.mobile : LOGO.url.desktop}
        />
        ...
        <LogoContainer>
          <img
            style={{ flex: 1 }}
            src={
              "https://s3.amazonaws.com/cdn.sonyafalcon.com/logo-landing.svg"
            }
          />
        </LogoContainer>
