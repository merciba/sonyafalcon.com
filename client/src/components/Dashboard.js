...
class Dashboard extends Component {
    ...
    this.props
      ...
      .then(res => {
        ...
        if (res.data.statusCode === 401)
          console.log(res.data); // window.location.replace('/login')
  ...
  prerender(post) {
    return axios({
      method: "post",
      url: `${window.config.post}/prerender`,
  ...
  renderPostBox() {
    ...
    return posts.map(post => (
      <div
        key={post.id}
        className="card"
        style={{ margin: 10, height: 460, width: 400 }}
      >
        ...
        <div className="card-content">
          <div className="media">
            <div className="media-content">
              <p className="title is-4">
                {post.published ? post.title : `${post.title} [Draft]`}
              </p>
            </div>
          </div>
          <div className="content">
            ...
            <br />
            <time dateTime={moment(post.timestamp).format("YYYY-MM-DD")}>
            ...
            <ButtonsContainer>
              <button
                ...
                onClick={() => window.location.replace(`/${post.id}`)}
              >
                View
              </button>
              <button
                style={{ flex: 1, maxWidth: 75 }}
                className="button level-item is-info"
                onClick={() => window.location.replace(`/editor/${post.id}`)}
              >
                Edit
              </button>
              <button
  ...
  render() {
    return this.state.loading ? (
      ...
      <section className="section" style={{ height: window.innerHeight }}>
        <Notifications />
