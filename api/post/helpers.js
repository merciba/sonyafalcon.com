...
const getCloudFrontId = () => {
  return cloudfront
    ...
    .promise()
    .then(distributions =>
      _.find(distributions.DistributionList.Items, {
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(`${__dirname}/post.hbs`, "utf-8", (error, source) => {
      ...
      s3.putObject(
        {
          Body: html,
          Bucket: "sonyafalcon.com",
          Key: `client/dist/${post.id}`,
            ...
            s3.putObject(
              ...
              err => {
                if (err) reject(err);
                  ...
                  resolve({ msg: `Successfully rendered post '${post.id}'` });
              }
            );
          }
        }
...
const createPost = event => {
  return new Promise((resolve, reject) => {
    axios({
      method: "post",
      url: `${process.env.USER_API}/authenticate`,
  ...
  return new Promise((resolve, reject) => {
    const update = () => {
      let params = {
        ...
        UpdateExpression:
          "set title = :title, page = :page, titleImg = :titleImg, dates = :dates, html = :html, layout = :layout, category = :category, subcategory = :subcategory, scheduled = :scheduled, tags = :tags, details = :details, published = :published",
        ExpressionAttributeValues: {
          ":title": event.body.title,
          ":page": event.body.page,
          ":category": event.body.category,
          ...
          getCloudFrontId()
            ...
            .then(res =>
              render(
                event.body,
                parseInt(
                  res.DistributionConfig.DefaultRootObject.replace(
                    "index-",
                    ""
                  ).replace(".html", "")
                ),
            ...
            .then(() =>
              resolve({ msg: `Successfully updated post '${event.body.id}'` })
            );
        }
      });
    };
                  ...
                  s3.deleteObject(
                    ...
                    err => {
                      if (err) reject(err);
                        ...
                        resolve({
                          msg: `Successfully deleted post '${event.body.id}'`
                        });
                    }
                  );
                }
...
const publish = event => {
  return getPost(event).then(result => {
    ...
    return getCloudFrontId()
      ...
      .then(() => {
        ...
        sgMail.send({
          ...
          text: `You published '${
            post.title
          }'. View it at https://sonyafalcon.com/${post.id}.`,
          html: `<h1>You published '${
            post.title
          }'.</h1><p>View it at https://sonyafalcon.com/${post.id}.</p>`
        });
...
const publishScheduled = event => {
  return getPosts(event).then(results => {
    return Promise.map(results, result => {
      if (
        !result.published &&
        result.scheduled &&
        moment(result.scheduled).valueOf() < moment().valueOf()
      ) {
