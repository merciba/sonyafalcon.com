...
module.exports.authenticate = (event, context, callback) => {
  try {
    if (event.headers && event.headers.Authorization) {
      axios.get(`https://www.googleapis.com/oauth2/v3/tokeninfo?id_token=${event.headers.Authorization}`)
        .then((res) => {
          let params = {
...
module.exports.createUser = (event, context, callback) => {
  try {
    if (event.headers && event.headers.Authorization) {
      axios.get(`https://www.googleapis.com/oauth2/v3/tokeninfo?id_token=${event.headers.Authorization}`)
        .then((res) => {
          if (res.data.email === process.env.ADMIN_EMAIL) { // Remove this conditional expression to allow non-admin users.
            let params = {
              TableName: process.env.DYNAMODB_TABLE,
              Item: {
                id: res.data.email,
            ...
            dynamodb.put(params, (err) => {
              if (err) fail(context, 500, new Error(err))
              else succeed(context, { msg: `Successfully created user ${res.data.email}` })
            })
          } else fail(context, 401, new Error('Unauthorized'))
        })
        .catch(() => fail(context, 401, new Error('Unauthorized')))
            ...
            if (err) fail(context, 500, new Error(err))
              ...
              dynamodb.update(updateParams, (err, result) => {
                if (err) fail(context, 500, new Error(err))
                  ...
                  succeed(context, { msg: `Successfully authenticated user`, email: res.data.email })
                }
              })
            }
          })
...
module.exports.addEmailToList = (event, context, callback) => {
  try {
    axios({ url: 'https://api.sendgrid.com/v3/contactdb/recipients', headers: { Authorization: `Bearer ${process.env.SENDGRID_API_KEY}` }, method: 'post', data: [event.body] })
      .then((res) => succeed(context, { msg: `User ${event.body.email} successfully subscribed to newsletter.` }))
      .catch(() => fail(context, 401, new Error('Unauthorized')))
  } catch (err) {
    fail(context, 500, new Error(err))
  }
